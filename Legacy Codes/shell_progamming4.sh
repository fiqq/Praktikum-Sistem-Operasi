...
while [[ loop -eq 1 ]]; do
    ...
    echo "Input Nama mahasiswa";
        ...
        read npm[$index_mhs];
    echo "Input jurusan mahasiswa";
        read jurusan[$index_mhs];
    echo "Input semester mahasiswa";
        read semester[$index_mhs];
        let index_mhs=$index_mhs+1;
            ;;
        "2" )
    echo "Input npm mahasiswa yang ingin di inputkan mataa kuliahnya";
        read npm;
    for (( i = 0; i < $index_mhs; i++ )); do
      if [[ npm -eq npm[$i] ]]; then
        echo "NILAI MATA KULIAH ATAS NAMA : ${nama[$i]}";
        echo "Nilai Sisop : "
            read mk1[$index_mk1];
